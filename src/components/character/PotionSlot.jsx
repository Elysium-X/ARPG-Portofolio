...
function PotionSlot({ emoji, name, color, hotkey, tooltipClass }) {
  return (
    <div className="group relative w-16 h-16 md:w-20 md:h-20 bg-gray-950 rounded-sm border-2 border-gray-700 shadow-inner transition-all duration-300 hover:border-gold/50 cursor-pointer">
      ...
      {/* Hotkey Indicator */}
      {hotkey && (
        <div className="absolute top-1 left-1 text-[0.625rem] font-pixel text-gray-500 group-hover:text-gold z-10 transition-colors">
          {hotkey}
        </div>
      )}
      ...
      {/* Tooltip (Simplified) */}
      <div className={`absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-black/90 text-gold ${tooltipClass || 'text-xs'} border border-gray-700 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 font-pixel tracking-wide`}>
        {name}
      </div>
    </div>
  )
